...
import { deleteFooterMenu, getFooterMenus, updateOrderOfMenu } from '@/lib/adminApi';
...
export default function HeaderData({ activeKey, reload, onEdit }: Props) {
    ...
    const handleDelete = async (item: any) => {
        try {
            const res = await deleteFooterMenu({ menuId: item._id });
            if (res.status === true) {
    ...
    const fetchData = async () => {
        try {
            const res = await getFooterMenus();
            if (res.status === true) {
