...
footerMenuSchema.pre('save', async function (next) {
    ...
    if (doc.isNew) {
        try {
            ...
            const maxOrder = await footerMenuModel.findOne().sort('-order').select('order').lean();
...
const footerMenuModel = mongoose.model('footerMenu', footerMenuSchema);

module.exports = footerMenuModel;
