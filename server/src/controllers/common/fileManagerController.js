...
const fileManagerController = {
    ...
    getListOfContributors: async (req, res) => {
        try {
            ...
            if (!files || files.length === 0) {
                return res.status(404).json({ status: false, message: 'No files found in the specified folder and its subfolders' });
            }
    ...
    getFilesWithParams: async (req, res) => {
        try {
            ...
            const files = await FileManagerFile.find(query, null, options)
                .populate([
                    {
                        path: 'createdBy',
                        select: 'email name image',
